...
void main() {
  ...
  testWidgets('shared error view exposes retry action', (
    WidgetTester tester,
  ) async {
    bool retried = false;
    await tester.pumpWidget(
      harness(
        AppErrorView(message: 'Fallo de red', onRetry: () => retried = true),
      ),
    );
  ...
  testWidgets('shared empty and loading views render accessible content', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      harness(
  ...
  testWidgets('page scaffold renders contextual app bar', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      harness(const PageScaffold(title: 'Productos', child: Text('Contenido'))),
    );
