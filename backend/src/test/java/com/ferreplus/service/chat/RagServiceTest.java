...
class RagServiceTest {
    ...
    void search_returnsTopFiveDocumentsAndBuildsSources() {
        ...
        when(repository.findNearest(any(float[].class), eq(5))).thenReturn(List.of(document));
        ...
        verify(repository).findNearest(same(vector), eq(5));
    }
    ...
    void search_emptyCorpus_returnsEmptyContextAndSources() {
        ...
        when(repository.findNearest(any(float[].class), eq(5))).thenReturn(List.of());
